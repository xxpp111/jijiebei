...
| `docs/wiki/bootstrap.json` | 当前 81 项 Markdown disposition 明细；账内保留 105 条历史 boot 快照（初始 78 条 + 27 次 canonical 内容刷新），另有 2 项与 register 行同 id 以 REPLAYED 跳过；coverage 由本文件显式界定 |
...
- 2026-08-26 加密恢复入口刷新时点：current bootstrap dispositions=80，boot 历史=85（初始 78 + 7 个 canonical refresh），提交态 Markdown coverage=84。
- 2026-08-27 搬迁手册刷新终态：current bootstrap dispositions=81；账内 boot 历史=91（初始 78 + 13 个 canonical refresh）；提交态 Markdown coverage=85。bootstrap current、历史 boot 与 Git coverage 是三种不同口径，不互相替代。
- 2026-08-27 搬迁手册终审修正终态：migration 的 fail-fast/目标架构修正与本协议页最终投影各追加新 rev；current bootstrap dispositions 仍为 81，账内 boot 历史=93（初始 78 + 15 个 canonical refresh），提交态 Markdown coverage=85。
- 2026-08-27 搬迁命令语义收口终态：deploy skill、deployment、migration 与本协议页各追加新 rev；current bootstrap dispositions 仍为 81，账内 boot 历史=97（初始 78 + 19 个 canonical refresh），提交态 Markdown coverage=85。
- 2026-08-27 搬迁 WAL/可移植部署最终收口：deploy skill、deployment、migration 与本协议页各再追加新 rev；current bootstrap dispositions 仍为 81，账内 boot 历史=101（初始 78 + 23 个 canonical refresh），提交态 Markdown coverage=85。
- 2026-08-27 搬迁重试路径最终收口：migration 的 operator/server 初始化与 source/runtime 分离修订及本协议页各追加新 rev；current bootstrap dispositions 仍为 81，账内 boot 历史=103（初始 78 + 25 个 canonical refresh），提交态 Markdown coverage=85。
- 2026-08-27 nginx 架构终验收口：migration 的目标镜像 inspect/pull/assert/smoke 修订及本协议页各追加新 rev；current bootstrap dispositions 仍为 81，账内 boot 历史=105（初始 78 + 27 个 canonical refresh），提交态 Markdown coverage=85。
...
7. 2026-08-27 搬迁手册 bootstrap refresh：新增 migration current item，并为 root/docs README、operations、deploy skill 与本协议页各追加新 rev；内容冻结后由一次 registrar transaction 写入，旧 boot 行不追改。
8. 2026-08-27 搬迁手册终审修正 refresh：为 migration 的 fail-fast/目标架构修订与本协议页最终投影各追加新 rev；旧 `boot-89`/`boot-91` 保留为历史，current item 只指向新修订。
9. 2026-08-27 搬迁命令语义收口 refresh：为 deploy skill、deployment、migration 的双向迁移/停写/归档/不可变 SHA/参数化部署修订与本协议页投影各追加新 rev；旧行只保留历史。
10. 2026-08-27 搬迁 WAL/可移植部署 refresh：为 deployment/migration 的 WAL 感知与 clean `TARGET_SHA` build、deploy skill/migration 的代理/前端/多架构 fallback 修订及本协议页投影各追加新 rev；旧行只保留历史。
11. 2026-08-27 搬迁重试路径 refresh：为 migration 的独立 operator/server 初始化与 `jijiebei-source`/`jijiebei-deploy` 分离修订及本协议页投影各追加新 rev；旧行只保留历史。
12. 2026-08-27 nginx 架构终验 refresh：为 migration 的 cached-image 架构 inspect、目标平台 pull、post-pull assert 与 smoke 修订及本协议页投影各追加新 rev；旧行只保留历史。
