...
### 2.3b 落库触发时机（#94 修复后，主播视角）

**判完第 3 场，5 秒后自动落库，不需要打开结算屏。** 2026-07-04 那场比赛 53 个报名选手对局全 0 落库，根因是旧版本只在结算屏（ResultScreen）挂载时才触发自动 POST——主播直播用 OBS 横条收尾、从不点进结算屏，数据就永远没进库。修复后：

- **触发点**：Battle 屏（判定屏）判完 3 场即在后台自动尝试落库，带 5 秒改判缓冲（这段时间内改判不会提前发、也不会重复发第二条）。
- **显式反馈**：Battle 屏页脚常驻状态条——灰字「比赛完成后自动录入天梯」→「录入中…」→「✓ 战绩已录入天梯」；失败会显示「⚠ 录入失败 · 点击重试」，可直接点击重跑，不需要刷新页面。
- **身份前置检查**：比赛模式下若当前账号不是主播（`kind=host`）登录，Battle 屏会常驻「⚠ 当前身份无法录入天梯，需主播账号」；首页开局那一步也会弹一次性软提示，提醒非主播账号本局战绩不会进天梯（但仍允许开局，不阻断直播流程）。
- **结算屏仍保留**兜底自动落库 + 手动「确认录入」按钮（失败重试用），与 Battle 屏的自动落库共用同一条防重逻辑（按局内容生成指纹，不因改判或先后触发而重复建档）。
- **练习模式**（`mode=practice`）行为不变：仍走原静默自助落库，Battle 屏不显示状态条。
