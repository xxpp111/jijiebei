# 集结杯 · 体系运作图（SYSTEM MAPS）【已归档】

> **归档声明（2026-07-03 R3 文档合并）**：本文件是 2026-06-24 的体系快照，不再维护。
> 图 1（系统架构）与图 2（比赛流程）的现行版在 `docs/architecture.md` §0 / §3（已更新到
> 7 集合；图 2 里的「天梯恒空」断点当时即已修复）；图 3（开发体系）的现行真相源是
> `AGENTS.md` + 全局 agent-dispatch skill。渲染 PNG（jjb-map-1/2/3.png）已随归档删除，
> 需要时用下方 Mermaid 源再生。
>
> 原头注：repo 内 Mermaid 版（进 git、可 diff、随代码演进）。团队可读版在飞书 KB 画板。
