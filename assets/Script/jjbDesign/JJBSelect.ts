...
// 本版=结构高保真静态渲染（真实图标）+ 完整拖拽吸附：池子因子/指挥官拖到场次目标槽，
// 复用 jijie2 SelectPanel.checkHit（<30px）做吸附判定；命中填槽并写本地选择状态（window.__jjbDebug.select），
// 未命中 tween 回弹。选择状态留在 jjbDesign（DEMO 数据），真实 JijieData 联调见 P5。
import { Theme } from "./JJBTheme";
import { EVENT, DEMO_MATCHES, FACTORS, GROUP_A, GROUP_B, POOL, MARK, FONT_NUM } from "./JJBData";
...
export default class JJBSelect {
    ...
    static build(root: cc.Node, th: Theme): void {
        ...
        // 本地选择状态（不写 jijie2；P5 再接真实 JijieData）。
        ...
        const updateDebug = () => {
            try {
                ...
                w.__jjbDebug.select = { slots: selection.slots.map((s: any) => ({ cmds: s.cmds.slice(), factors: s.factors.slice() })) };
            } catch (e) { /* noop */ }
        };

        // 填充某个目标槽（前置渲染、拖拽命中均走此一处）。
        ...
        for (let i = 0; i < 3; i++) {
            ...
            if ((m as any).doubles) { JJBView.box(root, sx + colW - 96, slotsTop + 12, 44, 22, null, th.accent, 1); JJBView.label(root, sx + colW - 96, slotsTop + 15, 44, 16, "双打", 12, th.accent, HA.CENTER); }
            ...
            for (let k = 0; k < cmdN; k++) {
                ...
                targets.push(t);
                if (filled) fillTarget(t, m.cmds[k]);
            }
            const facN = Math.max(3, m.factors.length);
            for (let k = 0; k < facN; k++) {
                const fxx = sx + 14 + (k % 5) * 56;
                const fyy = ty + 70 + Math.floor(k / 5) * 56;
                const filled = active && !!m.factors[k];
                let hint: cc.Node = null;
                if (!filled) { JJBView.box(root, fxx, fyy, 50, 50, th.panelBg, th.dropEdge, 1); if (!active && k === 0) hint = JJBView.label(root, fxx, fyy + 18, 50, 14, "因子", 11, th.muted, HA.CENTER); }
                ...
                targets.push(t);
                if (filled) fillTarget(t, m.factors[k]);
            }
        }
        ...
        FACTORS.forEach((f, i) => {
            ...
            if (i < 3) { JJBView.box(root, fx - 2, fy - 2, 70, 70, null, th.accent, 2); JJBView.box(root, fx + 52, fy - 10, 22, 22, th.accent, null); JJBView.label(root, fx + 52, fy - 7, 22, 16, "✓", 13, th.onAccent, HA.CENTER); }
        ...
        GROUP_A.forEach((c, i) => makeDraggable(JJBView.coverSprite(root, cmdX + i * 78, poolTop + 26, 66, 79, "images/commander/" + c), "cmd", c));
        JJBView.label(root, cmdX + 280, poolTop - 4, 200, 22, "B 组指挥官", 18, th.ink);
        GROUP_B.forEach((c, i) => makeDraggable(JJBView.coverSprite(root, cmdX + 280 + i * 78, poolTop + 26, 66, 79, "images/commander/" + c), "cmd", c));
        ...
        updateDebug(); // 初始（含 active 场预填）即暴露选择状态
    }
}
