...
export default class JJBHome {
    ...
    static build(root: cc.Node, th: Theme, onMode?: (i: number) => void): void {
        ...
        JJBView.sprite(root, 38, 30, 125, 100, markPath);          // 金 logo 100高(超 v3 88，联名更突出)
        JJBView.box(root, 181, 43, 3, 74, th.accent);              // 竖线 3×74
        // 标题艺术字整图（PIL 预渲染，放大）
        const titleW = th.style === "sc2" ? 599 : th.style === "minimal" ? 666 : 585;
        const tH = 84, tW = Math.round(tH * titleW / 200);
        JJBView.sprite(root, 204, 30, tW, tH, "images/brand/jjb-title-" + th.style + "-" + th.mode);
        JJBView.label(root, 208, 120, 300, 22, EVENT.en, 16, th.accent, HAlign.LEFT, 255, FONT_NUM);

        // ---------- 副标 / 主办 ----------
        JJBView.label(root, 38, 152, 700, 28, EVENT.sub, 20, th.muted);
        JJBView.label(root, 38, 182, 900, 24, EVENT.org + "　·　B站主播「" + EVENT.host + "」", 16, th.muted);

        // ---------- 选手输入 ----------
        JJBView.label(root, 38, 226, 100, 28, "参赛选手", 20, th.ink);
        JJBView.box(root, 150, 218, 470, 54, th.panelBg, th.panelEdge, 1);
        JJBView.label(root, 168, 235, 300, 24, "请输入选手 ID", 17, th.muted);
        JJBView.box(root, 290, 232, 2, 26, th.accent); // caret

        // ---------- 比赛模式 ----------
        JJBView.label(root, 38, 300, 120, 18, "SELECT MODE", 13, th.accent, HAlign.LEFT, 255, FONT_NUM);
        JJBView.label(root, 144, 292, 200, 32, "比赛模式", 25, th.ink);

        const gridLeft = 38, gridTop = 338, colW = 389, rowH = 104, gapX = 18, gapY = 18;
        MODES.forEach((m, i) => {
        ...
        // ---------- 页脚 ----------
        JJBView.label(root, 38, 676, 360, 22, EVENT.org, 15, th.ink);
        const linkStr = EVENT.links[0].k + " " + EVENT.links[0].v + "      开黑群 " + EVENT.qq;
        JJBView.label(root, 700, 676, 542, 22, linkStr, 15, th.muted, HAlign.RIGHT);
    }
    ...
    private static modeBtn(root: cc.Node, th: Theme, bx: number, by: number, w: number, h: number,
        ...
        JJBView.label(root, bx + 84, by + 39, 240, 32, name, 25, th.ink);
        // 标签
        const tagW = 60;
