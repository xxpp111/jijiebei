...
export default class JJConfigData {
    ...
    public static calcCurrentScore(bones: Boolean): number {
        var baseScore: number = Number(JJConfigData.commanderList[GameData.battleCommander][2]);
        var boneScore: number = bones ? 1 : 0;
        var rate: number = 1;
        if (GameData.battleFactors.length == 2) {
            rate = JJConfigData.paramMap["双因子奖励"];
        } else if (GameData.battleFactors.length == 1) {
            rate = JJConfigData.paramMap["单因子奖励"];
        }

        return (baseScore + boneScore) * rate;
    }
