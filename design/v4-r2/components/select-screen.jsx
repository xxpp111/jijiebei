...
// ---------- 全局控制条 ----------
const CTRL_NAV = [["home", "首页"], ["select", "选择"], ["battle", "对战"], ["result", "结算"], ["overlay", "浮层"]];
function CtrlBar({ state = "full", styleName = "metal", mode = "dark", active = "select", reach = ["home", "select", "overlay"] }) {
  if (state === "collapsed") {
    return (
  ...
  return (
    <div className="ctrl">
      <div className="ctrl-g">
        {CTRL_NAV.map(([key, label]) => {
          const on = key === active;
          const off = !on && !reach.includes(key);
          return <button key={key} className={"ctrl-btn" + (on ? " on" : "") + (off ? " off" : "")}>{label}</button>;
        })}
      </div>
      {state === "confirm" ? (
