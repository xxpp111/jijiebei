...
(function () {
  ...
  /* 3 matches. wl = winLose code aligned to JijieData: 0 失败 / 1 胜利 / 2 带奖励.
     cmds is an array (1 = 单打, 2 = 双打). BOSS 战 demos 双打 + 更多因子. */
  const MATCHES = [
    { slot: '第 1 场', map: MAPS.korhal, cmds: [C.kerrigan], factors: [F.lava, F.void, F.zombie], wl: 1 },
    { slot: '第 2 场', map: MAPS.darkstar, cmds: [C.raynor], factors: [F.money, F.blizzard, F.nuke], wl: 2 },
    { slot: 'BOSS 战', map: MAPS.temple, cmds: [C.artanis, C.kerrigan], factors: [F.void, F.lava, F.nuke, F.zombie, F.money], wl: 1, boss: true, doubles: true },
  ];
  // winLoseList / winCount mirror real JijieData. winCount = 胜利(1) + 带奖励(2).
  const WL_CODE = ['lose', 'win', 'bonus'];          // index = wl code
  const WL_LABEL = ['失败', '胜利', '带奖励'];
  const winLoseList = MATCHES.map((m) => m.wl);
  const winCount = winLoseList.filter((w) => w === 1 || w === 2).length;
  ...
  // CM co-brand mark per style (dark-mode marks)
  const MARK = {
    metal: A + 'logo-cm-gold.png',
    sc2: A + 'logo-cm-silver.png',
    minimal: A + 'logo-cm-vector.png',
  };
  // light-mode variants: dark-metallic recolor, reads on white, keeps relief
  const MARK_LIGHT = {
    metal: A + 'logo-cm-metal-light.png',
    sc2: A + 'logo-cm-sc2-light.png',
    minimal: A + 'logo-cm-minimal-light.png',
  };
  ...
  window.JJB = { MODES, FACTORS, F, COMMANDERS, C, MAPS, MATCHES, WL_CODE, WL_LABEL, winLoseList, winCount, GROUP_A, GROUP_B, POOL, MARK, MARK_LIGHT, EVENT };
})();
