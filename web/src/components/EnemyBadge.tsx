...
export function EnemyBadge({ race, ai, size = 'lg', easterEggLabel }: { race?: RaceCode; ai?: string; size?: 'lg' | 'md' | 'sm'; easterEggLabel?: string }) {
  ...
  return (
    <span className={'menemy menemy-' + size} data-enemy-race={race ?? ''} data-screen-label="enemy-badge">
      ...
      {easterEggLabel && <span className="menemy-easter-egg" data-enemy-easter-egg="potato-friend" title={easterEggLabel}>{size === 'sm' ? '🥔' : easterEggLabel}</span>}
    </span>
  );
}
